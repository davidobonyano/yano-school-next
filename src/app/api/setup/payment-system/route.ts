...
export async function POST(request: Request) {
  try {
    ...
    const createPaymentFunctionSQL = `
    ...
    if (functionError) {
      ...
      const statements = createPaymentFunctionSQL.split(';').filter(s => s.trim());
      
      for (const statement of statements) {
        if (statement.trim()) {
          const { error } = await supabase.from('_temp').select('1').limit(0);
          // This will fail but we're just testing connection
        }
      }
    }
    ...
    return NextResponse.json({ 
      success: true,
      message: 'Payment system functions setup completed',
      note: 'If SQL execution failed, please run the SQL manually in Supabase dashboard'
    });
